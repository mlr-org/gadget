...
For **PD/ICE**, you can pre-compute effects with **iml** and pass them to `$fit()`, or pass a fitted **mlr3** learner as `model` and let gadget build ICE/PD internally.
For **ALE**, pass the fitted learner as `model`; gadget computes ALE internally (external ALE summaries are not accepted as `effect` yet).

The gadget workflow would be as follows:

1.  **Feature Effect Selection**: Choose a suitable feature effect method

2.  **Feature Effect Computation**: Obtain ICE/PD either externally (e.g. **iml**) or internally from the fitted learner; obtain ALE internally from the learner (gadget’s ALE path).
...
2. Pass a fitted `model` and let `gadget` compute ICE/PD internally.
```{r}
...
This is because gadget's splitting and node-wise heterogeneity require per-sample local ALE increments,
whereas current `iml` and `ALEPlot` outputs provide interval-level ALE summaries rather than sample-level effects.
Therefore, these outputs cannot currently be used as direct `effect` input for gadget.
...
```{r}
bike_tree_pd_effect = GadgetTree$new(strategy = PdStrategy$new(), n_split = 4)
